...
func (s *Server) serveSFTP(ch ssh.Channel) {
	...
	server, err := sftp.NewServer(ch,
		...
		sftp.WithDebug(debugWriter{log: s.log}),
	)
	if err != nil {
		s.log.Error("ssh: sftp server failed", slog.Any("err", err))
		exitSession(ch, 1)
		return
	}

	s.log.Debug("ssh: sftp subsystem started", slog.String("dir", shell.Dir()))
	...
	if err := server.Serve(); err != nil && !errors.Is(err, io.EOF) {
		s.log.Warn("ssh: sftp session ended", slog.Any("err", err))
		exitSession(ch, 1)
		return
	}
...
func (w debugWriter) Write(p []byte) (int, error) {
	w.log.Warn("ssh: sftp", slog.String("msg", strings.TrimRight(string(p), "\n")))
	return len(p), nil
}
